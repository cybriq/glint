...
func NewUnit(
	...
	go func() {
		log.D.Ln("run unit command loop", args)
		var e error
	out:
		for {
			select {
			case cmd := <-r.commandChan:
				switch cmd {
				case true:
					log.D.Ln(r.running.Load(), "run called for", args)
					if r.running.Load() {
						log.D.Ln("already running", args)
						continue
					}
					if r.worker != nil {
						if e = r.worker.Kill(); log.E.Chk(e) {
						}
					}
				...
				case false:
					...
					log.D.Ln("stop called for", args, running)
					if !running {
						log.D.Ln("wasn't running", args)
						continue
					}
					Kill(r.worker)
					// var e error
					...
					r.running.Store(false)
					stop()
					log.D.Ln(args, "after stop", r.running.Load())
				}
				break
			case <-r.quit.Wait():
				log.D.Ln("runner stopped for", args)
				break out
			}
		}
	}()
	...
	out:
		select {
		case <-quit.Wait():
			log.D.Ln("runner quit trigger called", args)
			running := r.running.Load()
			if !running {
				log.D.Ln("wasn't running", args)
				break out
			...
			Kill(r.worker)
			var e error
			if e = r.worker.Wait(); log.E.Chk(e) {
			}
			r.running.Store(false)
			stop()
			log.D.Ln(args, "after stop", r.running.Load())
